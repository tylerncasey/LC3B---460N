...
#define sr1 GetSR1MUX(CURRENT_LATCHES.MICROINSTRUCTION) ? ((CURRENT_LATCHES.IR >> 6) & 0x7) : ((CURRENT_LATCHES.IR >> 9) & 0x7)
#define dr GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION) ? 7 : ((CURRENT_LATCHES.IR >> 9) & 0x7)
...
void eval_micro_sequencer() {
  ...
  //microsequencer takes J, IRD, BEN, COND, R, IR[15:12] as inputs (R, BEN, IR are in the latch alr), J, Cond, and IRD come from microinstruction
  ...
  if(IRD == 1){ //check IRD for Jcode or opcode
    ...
    if(condCode == 1 && CURRENT_LATCHES.READY){
        NEXT_LATCHES.STATE_NUMBER = (Jcode | 2);
    }
    else if(condCode == 2 && CURRENT_LATCHES.BEN){
        NEXT_LATCHES.STATE_NUMBER = (Jcode | 4);
    }
    else if(condCode == 3 && ((CURRENT_LATCHES.IR>>11) & 1)){
        NEXT_LATCHES.STATE_NUMBER = (Jcode | 1);
    }
    else{
...
void eval_bus_drivers() { 
   ...
   */
    ...
    //PC only sends currPC val to BUS
    ...
    if(GetALUK(CURRENT_LATCHES.MICROINSTRUCTION) == 0){ //add
        if(steering){
            ...
            ALU_BUS = Low16bits(CURRENT_LATCHES.REGS[sr1] + CURRENT_LATCHES.REGS[sr2]);
        }
    }
...
void drive_bus() {
   ...
   * tristate drivers. 
   */
  //when there is data needed to be put on the bus, use data from eval bus driver and push data onto the bus
  //depending on gates
...
void latch_datapath_values() {
  ...
  //MDR can get data from mem or write to mem *************************need to do
