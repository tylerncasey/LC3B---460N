...
/* Signals generated by MEM stage and needed by previous stages in the
   pipeline are declared below. Note that I have all dependency stall signals*/
int v_mem_ld_reg, v_mem_ld_cc, TRAP_PC, TARGET_PC, mem_reg_id, mem_PC_mux;
...
void MEM_stage() {
  ...
  // write word logic
  int dcache_data_in; 
  int dcache_data_out = 0;
  int dcache_R = 0;

  int v_dcache_en = Get_DCACHE_EN(PS.MEM_CS) & PS.MEM_V;
  // need to check if instr actually accesses mem
  if (v_dcache_en) {
    // WE logic
    int WE0 = 0;
    int WE1 = 0;
    ...
    //if word feed all 16 bits, if byte duplicate value for even/odd address access
    dcache_data_in = (Get_DATA_SIZE(PS.MEM_CS)) ? PS.MEM_ALU_RESULT : (((PS.MEM_ALU_RESULT & 0xFF) << 8) + (PS.MEM_ALU_RESULT & 0xFF));
    dcache_access(PS.MEM_ADDRESS, &dcache_data_out, dcache_data_in, &dcache_R, WE0, WE1);
  }
    
    // dcache_out Logic block
    // since its already outputting a word only need to check if LDB stuff
  if (Get_DATA_SIZE(PS.MEM_CS) == 0) {
    if ((PS.MEM_ADDRESS & 1) == 0) {
      dcache_data_out = signExtension(dcache_data_out & 0xFF, 8);
    } else {
      dcache_data_out = signExtension((dcache_data_out >> 8) & 0xFF, 8);
    }
  }
  TRAP_PC = dcache_data_out;
  TARGET_PC = PS.MEM_ADDRESS;
  mem_stall = (!dcache_R) & v_dcache_en;  

  // pc_mux logic block (set PC_MUX)
  // gets IRnzp, cc, mem_v, as well as the BR, UNCOND, and TRAP
  ...
  // additional vals outputted to rest of pipeline
  ...
  //latching values to SR
  NEW_PS.SR_ADDRESS = PS.MEM_ADDRESS;
  NEW_PS.SR_DATA = dcache_data_out;
...
int v_agex_ld_reg, v_agex_ld_cc, agex_reg_id;
...
void AGEX_stage() {
  ...
  // mem_v logic / LD_MEM signal ****need to do
  // if prev state is stuck in mem dont go,

  // v_agex stuff / global signals
  ...
  // LD_MEM
  // shouldn't move forward if stuck in mem (mem_stall)
  LD_MEM = !mem_stall;
  ...
  if (LD_MEM) {
    ...
    if (Get_ADDR1MUX(PS.AGEX_CS)) {
      adder_in1 = PS.AGEX_SR1;
    } else {
      adder_in1 = PS.AGEX_NPC;
    }
    // addr2mux
    int addr2_mux_out;
    if (Get_ADDR2MUX(PS.AGEX_CS) == 0) {
      ...
      addr2_mux_out = signExtension(PS.AGEX_IR & 0x3F, 6);
    } else if (Get_ADDR2MUX(PS.AGEX_CS) == 2) {
      addr2_mux_out = signExtension(PS.AGEX_IR & 0x1FF, 9);
    } else {
      addr2_mux_out = signExtension(PS.AGEX_IR & 0x7FF, 11);
    }

    int adder_in2;
    if (Get_LSHF1(PS.AGEX_CS)) {
      adder_in2 = addr2_mux_out << 1;
    } else {
      adder_in2 = addr2_mux_out;
    }

    if (Get_ADDRESSMUX(PS.AGEX_CS)) {
      NEW_PS.MEM_ADDRESS = Low16bits(adder_in1 + adder_in2);
    } else {
      NEW_PS.MEM_ADDRESS = Low16bits((0xFF & PS.AGEX_IR) << 1);
    }

    // ALU area
    int sr2_out;
    if (Get_SR2MUX(PS.AGEX_CS)) {
      sr2_out = signExtension(PS.AGEX_IR & 0x1F, 5);
    } else {
      sr2_out = PS.AGEX_SR2;
    }

    if (Get_ALU_RESULTMUX(PS.AGEX_CS)) { // if ALU output selected
      if (Get_ALUK(PS.AGEX_CS) == 0) {   // add
        NEW_PS.MEM_ALU_RESULT = Low16bits(PS.AGEX_SR1 + sr2_out);
      } else if (Get_ALUK(PS.AGEX_CS) == 1) { // and
        NEW_PS.MEM_ALU_RESULT = Low16bits(PS.AGEX_SR1 & sr2_out);
      } else if (Get_ALUK(PS.AGEX_CS) == 2) { // xor
        NEW_PS.MEM_ALU_RESULT = Low16bits(PS.AGEX_SR1 ^ sr2_out);
      } else { // passB
        NEW_PS.MEM_ALU_RESULT = Low16bits(sr2_out);
      }
    } else { // if SHF output selected
      int nLS = (PS.AGEX_IR >> 4) & 1;
      int amount4 = (PS.AGEX_IR & 0xF);
      int MSB = (PS.AGEX_SR1 >> 15);
      int steering = ((PS.AGEX_IR >> 5) & 1);
      
      //00, 10 are LSHF
      //01 is RSHFL
      //11 is RSHFA
      if (!nLS) { //LSHF if IR[4] is 0
        NEW_PS.MEM_ALU_RESULT = Low16bits(PS.AGEX_SR1 << amount4);
      } else if(steering) { //now check IR[5] if 1 its RSHFA if not RSHFL
        if (MSB)
          NEW_PS.MEM_ALU_RESULT = Low16bits((PS.AGEX_SR1 >> amount4) | (0xFFFF << (16 - amount4)));
        else
          NEW_PS.MEM_ALU_RESULT = Low16bits(PS.AGEX_SR1 >> amount4);
      } else { // RSHFL
        NEW_PS.MEM_ALU_RESULT = Low16bits(PS.AGEX_SR1 >> amount4);
...
void DE_stage() {
  ...
  int LD_AGEX; /* You need to write code to compute the value of
                  LD.AGEX signal */
  // LD_AGEX
  // if there are dep_stalls we should not load, or if stuck because of
  // mem_stall
  LD_AGEX = !mem_stall;

  /* your code for DE stage goes here */
  CONTROL_STORE_ADDRESS = ((PS.DE_IR >> 10) & 0x3E) + ((PS.DE_IR >> 5) & 0x1);

  int sr1_id = (PS.DE_IR >> 6) & 0x7;
  int sr2_id;
  // DE.IR[13] is used to select SR2 ID
  if ((PS.DE_IR >> 13) & 1) {
  ...
  // sr1 needed and sr2 needed (all regs need to be checked to see if they are being used in other stages)
  // additionally the reg_ids need to match or else there is no dependence
  // for BR ops we need to check if cc are being updated in other states we need to stall
  if (PS.DE_V &&
      (((CONTROL_STORE[CONTROL_STORE_ADDRESS][0]) &&
        ((v_agex_ld_reg && (PS.AGEX_DRID == sr1_id)) ||
         (v_mem_ld_reg && (PS.MEM_DRID == sr1_id)) ||
         (v_sr_ld_reg && (PS.SR_DRID == sr1_id)))) ||
       ((CONTROL_STORE[CONTROL_STORE_ADDRESS][1]) &&
        ((v_agex_ld_reg && (PS.AGEX_DRID == sr2_id)) ||
         (v_mem_ld_reg && (PS.MEM_DRID == sr2_id)) ||
         (v_sr_ld_reg && (PS.SR_DRID == sr2_id)))) ||
       ((CONTROL_STORE[CONTROL_STORE_ADDRESS][12]) &&
  ...
  if (LD_AGEX) {
    ...
    // AGEX_DRID
    if (CONTROL_STORE[CONTROL_STORE_ADDRESS][2]) {
...
/************************* FETCH_stage() *************************/
void FETCH_stage() { /* your code for FETCH stage goes here */
  int LD_DE;         // load for next stage of pipeline
  int LD_PC;
  int nextPC = PC + 2;

  // icache access (PC is the addr, we get IR and R signal as outputs)
  int IR_temp;
  icache_access(PC, &IR_temp, &icache_r);

  // LD_PC logic
  LD_PC = (icache_r & !mem_stall & !v_de_br_stall & !v_agex_br_stall & !v_mem_br_stall & !dep_stall) | (v_mem_br_stall & (mem_PC_mux != 0));
  // PC_mux
  if (LD_PC) {
    if (mem_PC_mux == 2) {
      PC = TRAP_PC;
  ...
  // LD.DE
  // if dep_stall prev stage is stuck in DE, same with mem_stall as before
  LD_DE = !dep_stall && !mem_stall;

  if (LD_DE) { // latching for DE
    NEW_PS.DE_NPC = nextPC;
    NEW_PS.DE_IR = IR_temp;
