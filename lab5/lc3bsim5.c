...
int GetSTATEMUX(int *x) { return ((x[STATEMUX1] << 1) + x[STATEMUX0]); }
...
void mdump(FILE *dumpsim_file, int start, int stop) {
  ...
  for (address = (start >> 1); address <= (stop >> 1); address++)
    fprintf(dumpsim_file, " 0x%0.4x (%d) : 0x%0.2x%0.2x\n", address << 1,
            address << 1, MEMORY[address][1], MEMORY[address][0]);
  fprintf(dumpsim_file, "\n");
  fflush(dumpsim_file);
}
...
void rdump(FILE *dumpsim_file) {
  ...
  fprintf(dumpsim_file, "CCs: N = %d  Z = %d  P = %d\n", CURRENT_LATCHES.N,
          CURRENT_LATCHES.Z, CURRENT_LATCHES.P);
  ...
  for (i = 0; i < CONTROL_STORE_ROWS; i++) {
    ...
    for (j = 0; j < CONTROL_STORE_BITS; j++) {
      ...
      if (line[index] != '0' && line[index] != '1') {
        printf(
            "Error: Unknown value in micro-code file: %s\nLine: %d, Bit: %d\n",
            ucode_filename, i, j);
        exit(-1);
      }
...
void load_program(char *program_filename, int is_virtual_base) {
  ...
  if (is_virtual_base) {
    ...
    pte = (MEMORY[(CURRENT_LATCHES.PTBR +
                   (((program_base << 1) >> PAGE_NUM_BITS) << 1)) >>
                  1][1]
           << 8) |
          MEMORY[(CURRENT_LATCHES.PTBR +
                  (((program_base << 1) >> PAGE_NUM_BITS) << 1)) >>
                 1][0];
    ...
    if ((pte & PTE_VALID_MASK) == PTE_VALID_MASK) {
      program_base =
          (pte & PTE_PFN_MASK) | ((program_base << 1) & PAGE_OFFSET_MASK);
      printf("physical base of program: %x\n\n", program_base);
      program_base = program_base >> 1;
    } else {
      printf("attempting to load a program into an invalid (non-resident) "
             "page\n\n");
      exit(-1);
    }
  } else {
...
int main(int argc, char *argv[]) {
  ...
  /* Error Checking */
  if (argc < 4) {
    printf("Error: usage: %s <micro_code_file> <page table file> "
           "<program_file_1> <program_file_2> ...\n",
           argv[0]);
    exit(1);
  }
...
#define sr1                                                                    \
  (GetSR1MUX(CURRENT_LATCHES.MICROINSTRUCTION) == 2                            \
       ? 6                                                                     \
       : (GetSR1MUX(CURRENT_LATCHES.MICROINSTRUCTION) == 1                     \
              ? ((CURRENT_LATCHES.IR >> 6) & 0x7)                              \
              : ((CURRENT_LATCHES.IR >> 9) & 0x7)))
#define dr                                                                     \
  (GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 2                             \
       ? 6                                                                     \
       : (GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 1                      \
              ? 7                                                              \
              : ((CURRENT_LATCHES.IR >> 9) & 0x7)))
...
void eval_micro_sequencer() {
  ...
  // Compute combinatorial exceptions right now so they are instantly accurate
  // for this cycle
  int EXC;

  int isTRAPOP = (((CURRENT_LATCHES.IR >> 12) & 0xF) == 0xF) ? 1 : 0;

  int UE = (CURRENT_LATCHES.MAR & 1) && GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION);
  int PE = !((CURRENT_LATCHES.MDR >> 3) & 1) && CURRENT_LATCHES.PRIV && !isTRAPOP;
  int PF = !((CURRENT_LATCHES.MDR >> 2) & 1);
  if (CURRENT_LATCHES.STATE_NUMBER == 34) {
    EXC = UE;
  } else {
    EXC = PF || PE;
  }
  ...
  if (IRD == 1) { // check IRD for Jcode or opcode
    tempNextState = (CURRENT_LATCHES.IR >> 12) & 0x3F;
  } else {
    tempNextState = Jcode;
  ...
  if (GetLD_JREG(CURRENT_LATCHES.MICROINSTRUCTION)) {
    NEXT_LATCHES.JREG = tempNextState;
  }

  if (GetSTATEMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 3) {
    NEXT_LATCHES.STATE_NUMBER = 39;
  } else if (GetSTATEMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 2) {
    NEXT_LATCHES.STATE_NUMBER = 34;
  } else if (GetSTATEMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
    NEXT_LATCHES.STATE_NUMBER = CURRENT_LATCHES.JREG;
  } else {
    NEXT_LATCHES.STATE_NUMBER = tempNextState;
  }
...
void eval_bus_drivers() {
  ...
  } else if (CURRENT_LATCHES.MAR & 1) { // if loading byte need to check if addr
                                        // is even or odd (check LSB)
  ...
  // PSR BUS
  PSR_BUS = Low16bits((CURRENT_LATCHES.PRIV << 15) | (CURRENT_LATCHES.N << 2) |
  ...
  // PTBR BUS
  PTBR_BUS =
      (CURRENT_LATCHES.PTBR & 0xFF00) + ((CURRENT_LATCHES.VA & 0xFE00) >> 8);

  // VA BUS
  VA_BUS = (CURRENT_LATCHES.MDR & 0x3E00) + (CURRENT_LATCHES.VA & 0x1FF);
}
...
void latch_datapath_values() {
    ...
    if (memReady) { // need to check if mem is actually ready (also check datasize)
      if (GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION)) { // if datasize is 1 store whole word
        if(GetMDRMUX(CURRENT_LATCHES.MICROINSTRUCTION)){
          MEMORY[CURRENT_LATCHES.MAR >> 1][0] = CURRENT_LATCHES.PTE & 0xFF;
          MEMORY[CURRENT_LATCHES.MAR >> 1][1] = (CURRENT_LATCHES.PTE >> 8) & 0xFF;
        } else {
  ...
  if (GetLD_EXC(CURRENT_LATCHES.MICROINSTRUCTION)) {
    int TrapOP;
    if (((CURRENT_LATCHES.IR >> 12) & 0xF) == 0xF)
      TrapOP = 1;
    else
      TrapOP = 0;
    NEXT_LATCHES.PE = !((CURRENT_LATCHES.MDR >> 3) & 1) && CURRENT_LATCHES.PRIV && !TrapOP;
    NEXT_LATCHES.PF = !((CURRENT_LATCHES.MDR >> 2) & 1);
  ...
  if (GetLD_VECT(CURRENT_LATCHES.MICROINSTRUCTION)) {
    if (GetVECTMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
      if (CURRENT_LATCHES.UE)
        NEXT_LATCHES.VECT = 0x03; // unaligned exception vector
      else if (CURRENT_LATCHES.PE)
        NEXT_LATCHES.VECT = 0x04; // protection exception vector
      else if (CURRENT_LATCHES.PF)
        NEXT_LATCHES.VECT = 0x02; // page fault vector
    } else if (GetVECTMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 2) {
      NEXT_LATCHES.VECT = 0x05; // unknown opcode exception vector
  ...
  if (GetLD_PTE(CURRENT_LATCHES.MICROINSTRUCTION)) {
    //if instr is a write set modify bit
    if(CURRENT_LATCHES.JREG == 23 || CURRENT_LATCHES.JREG == 24){
        NEXT_LATCHES.PTE = (CURRENT_LATCHES.MDR & 0xFFFC) | 0x3;  //set modify and reference bit
    } else {
        NEXT_LATCHES.PTE = (CURRENT_LATCHES.MDR & 0xFFFE) | 1;  //preserve what modify bit is
    }
  }
}
